...
def one_finger_touch_event_analysis(touch_event_df, classifier_name, print_results=False):
    ...
    if classifier_name == "k-NN":
        ...
        # 1. Without activity_id, session_number and start_timestamps, the best k value was 33 with 33.45% accuracy in a one shot test accuracy
        # 2. After adding the new properties down_down_duration_ms, up_down_duration_ms, X_coord_distance_avg and Y_coord_distance_avg, the accuracy increased to 34.02% (k = 24)
        # 3. After removing the previous added properties and added the following properties: hour_sin, hour_cos, dow_sin, dow_cos, month_sin, month_cos, is_weekend, part_of_day, down_up_duration_ms_avg, down_down_duration_ms_avg, up_down_duration_ms_avg, start_x, start_y, end_x, end_y, start_quadrant, end_quadrant, X_coord_distance, Y_coord_distance, direction, touch_length_euclidean_distance, touch_angle and contact_size_avg, the best k = 5 with 78.95% accuracy, 0.0107 FAR, 0.2104 FRR and 0.0478 EER with 0.2000 threshold
        # 4.  after removing move_actions_second, scenario, start_quadrant, end_quadrant, scenario and direction, the best k = 5 with 79.02% accuracy, 0.0106 FAR, 0.2097 FRR and 0.0464 EER with 0.2000 threshold
        ...
        # 1. After best_n_estimators=100, the accuracy is relatively constant at around 43%
        #   but best_n_estimators=190->200 showed the best accuracy of 43%+
        # 2. After adding the new properties down_down_duration_ms, up_down_duration_ms, X_coord_distance_avg and Y_coord_distance_avg, an increase of 2% in accuracy
        ...
        # 4.  after removing move_actions_second, scenario, start_quadrant, end_quadrant, scenario and direction, the best best_n_estimators = 201 with 88.91% accuracy, 0.0057 FAR, 0.1108 FRR and 0.0220 EER with 0.1563 threshold
        ...
        # 4.  after removing move_actions_second, scenario, start_quadrant, end_quadrant, scenario and direction, tried the arbitrary values of: svm__C: 100, svm__gama: 0.01 and svm__kernel: rbf with 77.14% accuracy, 0.0116 FAR, 0.22285 FRR and 0.0572 EER with 18.74 threshold
    else:
